...
- `src/services/nutritionPlansService.ts` עובד באותו pattern ומפנה ל-`nutritionPlansApiService.ts` כשמכבים את הדגל.
...
- Users, Workout Plans ו-Nutrition Plans עוברים בין Mock ל-API לפי אותו flag.
- מעבר כזה לא דורש שינוי ב-hook או במסך.
- בעתיד אפשר להרחיב את אותו pattern לדומיינים נוספים באותו סדר.

## מה Backend צריך לספק

- Endpoints תואמים ל-Users, ל-Workout Plans ול-Nutrition Plans contracts.
